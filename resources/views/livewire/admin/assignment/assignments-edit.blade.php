<form method="POST" wire:submit.prevent="updateAssignment">
    @csrf
    ...
    <select wire:model="id_branch" id="id_branch"
        ...
        <option value="" disabled selected>Seleccionar una desconsentrada...</option>
    ...
    <!-- Técnicos Asignados -->
    <div class="mb-4">
        <label class="block text-sm font-medium text-gray-700 mb-2">Técnicos Asignados</label>

        @if ($inactiveTechnicians->isEmpty())
            <p class="text-sm text-gray-500 italic">No hay técnicos asignados por el momento.</p>
        @else
            <div class="space-y-2">
                @foreach ($inactiveTechnicians as $technician)
                    <div class="flex items-center space-x-3">
                        <!-- Checkbox deshabilitado y marcado por defecto -->

                        <span class="text-sm text-gray-700">{{ $technician->name . ' ' . $technician->last_name }}</span>
                        <span class="text-xs font-semibold px-2 py-1 rounded-full bg-gray-100 text-gray-700">
                            Asignado
                        </span>

                        <!-- Botón para retirar al técnico -->
                        <button type="button" wire:click="removeTechnician({{ $technician }})"
